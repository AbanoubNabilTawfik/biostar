...
export class AuthService {
  ...
  // load the data
  loadToken() {
    if (localStorage.getItem('user') && JSON.parse(localStorage.getItem('lease_user')).token) {
      this.user = JSON.parse(localStorage.getItem('lease_user'));
    }
  }
  ...
  roleMatch(allowedRoles: string[]): boolean {
    ...
    let isMatch = false;
    let userRoles: string[] = JSON.parse(localStorage.getItem('lease_user')).userRoles;
